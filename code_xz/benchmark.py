...
def processDataMK(in_file_name, cust_dict, lag_cust_dict, lag_cust_dict2, lag_cust_dict3, lag_cust_dict4):
    x_vars_list = []
    y_vars_list = []
    ...
    for row in csv.DictReader(in_file_name):
        ...
        if row['fecha_dato'] == '2016-06-28':
            ...
            x_vars_list.append(x_vars + prev_target_list + lag_target_list + lag_target_list2 + lag_target_list3 +
                               lag_target_list4)
            ...
            if sum(new_products) > 0:
                for ind, prod in enumerate(new_products):
                    if prod > 0:
                        ...
                        x_vars_list.append(x_vars + prev_target_list + lag_target_list + lag_target_list2 +
                                           lag_target_list3 + lag_target_list4)
                        y_vars_list.append(ind)

    return x_vars_list, y_vars_list, cust_dict, lag_cust_dict, lag_cust_dict2, lag_cust_dict3, lag_cust_dict4

def cvXGB(X, y, seed_val=0):
    NFOLDS = 16
    ...
    for i, (train_fold, validate) in enumerate(kf):
        if i == 2:
            ...
            bst = xgb.train(param, dtrain, num_rounds, evals=watchlist, verbose_eval=10, early_stopping_rounds=50)
            best_trees = bst.best_ntree_limit
            return param, int(best_trees * (1. + 1. / NFOLDS))

def runXGB(train_X, train_y, param, num_rounds):
    xgtrain = xgb.DMatrix(train_X, label=train_y)
    watchlist = [(xgtrain, 'train')]
    model = xgb.train(param, xgtrain, num_rounds, evals=watchlist, verbose_eval=10)
    return model
...
if __name__ == "__main__":
    ...
    x_vars_list, y_vars_list, cust_dict, lag_cust_dict, lag_cust_dict2, lag_cust_dict3, lag_cust_dict4 = processDataMK(
        train_file, {}, {}, {}, {}, {})
    ...
    x_vars_list, y_vars_list, cust_dict, lag_cust_dict, lag_cust_dict2, lag_cust_dict3, lag_cust_dict4 = processDataMK(
        test_file, cust_dict, lag_cust_dict, lag_cust_dict2, lag_cust_dict3, lag_cust_dict4)
    ...
    param, num_rounds = cvXGB(train_X, train_y)
    print("Building model..")
    model = runXGB(train_X, train_y, param, num_rounds)
    del train_X, train_y
    print("Predicting..")
    ...
    out_df.to_csv('sub_xgb_benchmark.csv', index=False)
    print(datetime.datetime.now() - start_time)